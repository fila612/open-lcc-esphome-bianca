...
- **More reliable shot counting and water-tank handling**: persistent shot counters across reboots, a minimum-shot-duration filter to keep flushes out of the count, and a flag that discards a "brew" if the tank ran dry mid-shot so it isn't counted or timed.
...
Progress toward that parity goal is tracked via the project version in `openlcc.yaml` (currently 0.9.1; 1.0 marks full parity with the original LCC firmware, confirmed on the machine).
